...
const NavbarMobile = () => {
  ...
  useEffect(() => {
    const BTN_NAV_SOBRE = document.getElementById('mobile_link_sobre');
    const BTN_NAV_FORMACAO = document.getElementById('mobile_link_formacao');
    const BTN_NAV_HABILIDADES = document.getElementById('mobile_link_habilidades');
    const BTN_NAV_PROJETOS = document.getElementById('mobile_link_projetos');
    const BTN_NAV_CONTATO = document.getElementById('mobile_link_contato');
  ...
  return (
    <>
      <div className="NavbarMobile">
        <div className={'btnFecharNavMobile '+(navAberto === true ? "aberto" : "fechado")} onClick={handleNavToggle}>
          <span className='iconeBarra barra01'></span>
          <span className='iconeBarra barra02'></span>
          <span className='iconeBarra barra03'></span>
        </div>
      </div>

      <div className={"navbarBotoesMobile "+(navAberto === true ? "aberto" : "fechado")}>
        <a id="mobile_link_sobre" href="#secaoApresentação" onClick={scrollToSobre} className='font-h5 link selected'>Sobre</a>
        <a id="mobile_link_formacao" onClick={scrollToFormacao} className='font-h5 link'>Formação</a>
        <a id="mobile_link_habilidades" href="#secaoHabilidades" onClick={scrollToHabilidades} className='font-h5 link'>Habilidades</a>
        <a id="mobile_link_projetos" href="#secaoProjetos" onClick={scrollToProjetos} className='font-h5 link'>Projetos</a>
        <a id="mobile_link_contato" href="#secaoContato" onClick={scrollToContato} className='font-h5 link'>Contato</a>
      </div>
    </>
  )
}
